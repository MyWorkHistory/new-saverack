...
class WholesaleOrderController extends Controller
{
     ...
     * Parses inline for validation, then applies in the background (avoids Cloudflare 524).
     */
    public function importLinesCsv(
        Request $request,
        WholesaleOrder $wholesaleOrder,
        \App\Services\WholesaleOrderLinesCsvImportService $csv
    ): JsonResponse {
        Gate::authorize('update', $wholesaleOrder);
        $this->assertLineEditable($wholesaleOrder);
        ...
        if ($rows !== []) {
            \App\Jobs\ImportWholesaleOrderLinesCsvJob::dispatchAfterResponse(
                (int) $wholesaleOrder->id,
                $rows
            );
        }
        ...
        $message = $queued > 0
            ? "Upload received. Importing {$queued} SKU".($queued === 1 ? '' : 's').' in the background.'
            : 'No valid SKU rows to import.';
