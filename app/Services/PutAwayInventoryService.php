...
class PutAwayInventoryService
{
    ...
    public function list(
        ...
        if ($refresh || $snapshot === null || ! $this->isFresh($snapshot)) {
    ...
    private function rebuildSnapshot(int $clientAccountId, ClientAccount $account, string $customerId): PutAwaySnapshot
    {
        ...
        try {
            ...
            do {
                ...
                $listRows = is_array($page['rows'] ?? null) ? $page['rows'] : [];
                foreach ($listRows as $listRow) {
                    if (! is_array($listRow)) {
                        continue;
                    }
                    $sku = trim((string) ($listRow['sku'] ?? ''));
                    if ($sku === '') {
                        continue;
                    }
                    if (($listRow['kit'] ?? false) === true || ($listRow['kit_build'] ?? false) === true) {
                        continue;
                    }
                    ...
                    $locations = PutAwayRowBuilder::locationsFromProductDetail(is_array($product) ? $product : null);
                    $onHand = (int) round((float) ($listRow['on_hand'] ?? 0));
                    if ($onHand <= 0 && is_array($product)) {
                        $metrics = is_array($product['metrics'] ?? null) ? $product['metrics'] : [];
                        $onHand = (int) ($metrics['on_hand'] ?? 0);
                    }
                    $backorder = (int) round((float) ($listRow['backorder'] ?? 0));
                    if ($backorder <= 0 && is_array($product)) {
                        $metrics = is_array($product['metrics'] ?? null) ? $product['metrics'] : [];
                    ...
                    $builtRows[] = PutAwayRowBuilder::buildRow(
                        $sku,
                        (string) ($listRow['name'] ?? $sku),
                        isset($listRow['barcode']) ? (string) $listRow['barcode'] : null,
                        isset($listRow['image_url']) ? (string) $listRow['image_url'] : null,
                        $locations,
                        $onHand,
                        $backorder
                    );
