...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('client_account_shopify_connections', function (Blueprint $table) {
            $table->id();
            $table->foreignId('client_account_id')->unique()->constrained('client_accounts')->cascadeOnDelete();
        ...
        Schema::create('shopify_locations', function (Blueprint $table) {
            $table->id();
            $table->foreignId('connection_id')->constrained('client_account_shopify_connections')->cascadeOnDelete();
        ...
        Schema::create('shopify_product_variants', function (Blueprint $table) {
            $table->id();
            $table->foreignId('connection_id')->constrained('client_account_shopify_connections')->cascadeOnDelete();
            $table->foreignId('shopify_product_id')->constrained('shopify_products')->cascadeOnDelete();
        ...
        Schema::create('shopify_orders', function (Blueprint $table) {
            ...
            $table->index(['connection_id', 'fulfillment_status']);
            $table->index(['connection_id', 'shopify_created_at']);
        });

        Schema::create('shopify_order_line_items', function (Blueprint $table) {
            $table->id();
            $table->foreignId('connection_id')->constrained('client_account_shopify_connections')->cascadeOnDelete();
            $table->foreignId('shopify_order_id')->constrained('shopify_orders')->cascadeOnDelete();
        ...
        Schema::create('shopify_fulfillment_order_line_items', function (Blueprint $table) {
            $table->id();
            $table->foreignId('connection_id')->constrained('client_account_shopify_connections')->cascadeOnDelete();
            $table->foreignId('shopify_fulfillment_order_id')->constrained('shopify_fulfillment_orders')->cascadeOnDelete();
            $table->foreignId('shopify_order_line_item_id')->nullable()->constrained('shopify_order_line_items')->nullOnDelete();
        ...
        Schema::create('shopify_fulfillments', function (Blueprint $table) {
            ...
            $table->index(['connection_id', 'shopify_order_id']);
        });
        ...
        Schema::create('shopify_webhook_events', function (Blueprint $table) {
            ...
            $table->foreignId('connection_id')->nullable()->constrained('client_account_shopify_connections')->nullOnDelete();
            $table->json('payload');
            $table->timestamp('processed_at')->nullable();
            $table->string('processing_error', 500)->nullable();
            $table->timestamps();

            $table->index(['connection_id', 'processed_at']);
            $table->index('topic');
        });
    }
