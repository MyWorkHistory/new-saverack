...
const iconPath = computed(() => ICON_PATHS[props.icon] || ICON_PATHS.details);
</script>

<template>
  <div
    class="d-flex flex-wrap align-items-center justify-content-between gap-2 account-detail-section-head"
    :class="headClass"
  >
    <div class="account-detail-section-head__title-wrap min-w-0">
      ...
      <component :is="titleTag" :id="headingId" :class="titleClass">
        {{ title }}
      </component>
    </div>
    <slot name="actions">
      <button
        v-if="showEdit"
        type="button"
        class="btn btn-sm staff-account-edit-btn d-inline-flex align-items-center gap-1"
        @click="$emit('edit')"
      >
        <svg
          class="staff-account-edit-btn__icon"
          width="14"
          height="14"
          fill="none"
          stroke="currentColor"
