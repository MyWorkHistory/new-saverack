...
const shopifyGroupOpen = ref(route.path.startsWith("/admin/shopify"));
watch(
  () => route.path,
  (p) => {
    ...
    if (p.startsWith("/admin/shopify")) {
      shopifyGroupOpen.value = true;
    }
  },
);
...
function navActive(mode) {
  ...
  if (mode === "orders-batch") return p.startsWith("/admin/orders/batch");
  ...
  <aside :class="sidebarClass">
    ...
    <nav class="vx-sidebar__scroll">
      ...
      <ul class="list-unstyled mb-0 pb-2">
          ...
          <template v-if="isExpanded">
            <div>
              ...
              <ul v-show="ordersGroupOpen" class="list-unstyled mb-0 mt-1">
                ...
                <li v-if="canViewOrdersBatch">
                  <RouterLink
                    to="/admin/orders/batch"
                    class="vx-nav-link vx-nav-sublink"
                    :class="{ 'vx-nav-link--active': navActive('orders-batch') }"
                    @click="closeMobile"
                  >
                    Batch
                  </RouterLink>
        ...
        <li v-if="!isPortal && canViewReceiving">
          <template v-if="isExpanded">
            <div>
              <button
                type="button"
        ...
        <li v-if="canViewShopify">
          <template v-if="isExpanded">
            <div>
              <button
                type="button"
                class="vx-nav-link"
                :class="{ 'vx-nav-link--active': navActive('shopify') }"
                :aria-expanded="shopifyGroupOpen"
                @click="shopifyGroupOpen = !shopifyGroupOpen"
              >
                <svg
                  fill="none"
                  viewBox="0 0 24 24"
                ...
                <span class="text-truncate">Shopify</span>
                <svg
                  class="ms-auto flex-shrink-0 transition"
                  :class="shopifyGroupOpen ? 'rotate-180' : ''"
                  style="width: 1rem; height: 1rem"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
              ...
              <ul v-show="shopifyGroupOpen" class="list-unstyled mb-0 mt-1">
                <li>
                  <RouterLink
                    to="/admin/shopify/orders"
                    class="vx-nav-link vx-nav-sublink"
                    :class="{ 'vx-nav-link--active': navActive('shopify-orders') }"
                    @click="closeMobile"
                  >
                    Orders
                  </RouterLink>
                ...
                <li>
                  <RouterLink
                    to="/admin/shopify/inventory"
                    class="vx-nav-link vx-nav-sublink"
                    :class="{ 'vx-nav-link--active': navActive('shopify-inventory') }"
                    @click="closeMobile"
                  >
                    Inventory
                  </RouterLink>
