...
function applyReturnData(data) {
  ...
  if (String(data?.status || "").toLowerCase() === "pending") {
    selected.value = new Set((Array.isArray(data?.lines) ? data.lines : []).map((line) => line.id));
  } else {
...
async function processReturn() {
  ...
  try {
    ...
    if (returnFees.value.non_compliant != null) body.non_compliant_fee = returnFees.value.non_compliant;
    if (returnFees.value.photo != null) body.photo_fee = returnFees.value.photo;
