...
const billDetailFields = computed(() => {
  ...
  if (!b) return [];
  const fields = [
    { icon: "doc", label: "Bill Type", value: "Return Bill" },
  ...
  if (b.rma_number) {
    fields.push({
      ...
      label: "RMA",
      value: b.rma_number,
      sub: b.order_number ? `Order #${b.order_number}` : "",
      link: b.client_account_return_id
        ? {
            to: {
              name: "admin-process-return-detail",
              params: { id: String(b.client_account_return_id) },
            },
            label: "View Return",
          }
        : null,
